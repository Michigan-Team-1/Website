...
public class UsersCreateUpdate : BaseService
{
  ...
  public async Task<BaseServiceResponse<T>> SaveUser<T>(T dto) where T : UserDto
  {
    ...
    {
      var memberType = dbObj.UserMemberTypes.FirstOrDefault(w => w.MemberTypeId == item.MemberTypeId);
      if (memberType != null && item.IsDeleted)
      {
        db.UserMemberTypes.Remove(memberType);
        continue;
      }
      else if (memberType == null)
      {
        dbObj.UserMemberTypes.Add(new Model.UserIdentity.UserMemberType() { MemberTypeId = item.MemberTypeId });
      }
    }
    ...
    if (dto.UserRoles == null)
      dto.UserRoles = new List<UserRoleDto>();
    ...
    foreach (var item in dto.UserRoles)
    {
      // user doesn't have permission to grant this role skip it.
      if (!roles.Any(w => w.RoleId == item.RoleId))
        continue;

      var role = dbObj.UserRoles.FirstOrDefault(w => w.RoleId == item.RoleId);
      if (role != null && item.IsDeleted)
      {
      ...
      {
        dbObj.UserRoles.Add(new Model.UserIdentity.UserRole() { RoleId = item.RoleId });
      }
    }
    ...
    dto.UserMemberTypes = dto.UserMemberTypes.Where(w => !w.IsDeleted).Select(s =>
    {
      s.UserId = dbObj.UserId;
      return s;
    }).ToList();
    dto.UserRoles = dto.UserRoles.Where(w => !w.IsDeleted).Select(s => { s.UserId = dbObj.UserId; return s; }).ToList();
    dto.IsUpdated = false;
  ...
  public async Task<BaseServiceResponse<string>> SaveIpAddress(int userId)
  {
    ...
    var result = await db.SaveIpAddress(userId, UserPermissionService.IpAddress);
    if (result == 0)
    {
      response.Status = System.Net.HttpStatusCode.BadRequest;
