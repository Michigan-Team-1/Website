...
@Component({
  ...
  template: `<div class="form-group form-row">
      ...
      <div class="col-sm-9">
        <input type="text" id="{{predicate + nameSuffix}}" name="{{predicate + nameSuffix}}" [(ngModel)]="dto[predicate]" #stringMax="ngModel" class="form-control"
               [attr.maxlength]="propertyAttributes.stringLength.maxLength" [required]="propertyAttributes.required.value" />
        <div *ngIf="f.submitted && stringMax.invalid && (stringMax.dirty || stringMax.touched || f.submitted)" class="invalid-feedback d-block">
          <div *ngIf="stringMax.errors.required">
...
export class SpudStringMaxComponent implements OnInit {
  constructor(
  ) {
  }
