...
namespace Team1.Web.Site.Controllers
{
    ...
    public class BaseController : Controller
    {
        ...
        private DataContext _spudContext;
        /// <summary>
        /// Data context injected in so we can do transactions easier.
        /// </summary>
        public DataContext SpudContext
        {
            get
            {
                if (_spudContext == null)
                {
                    _spudContext = HttpContext.RequestServices.GetService(typeof(DataContext)) as DataContext;
                }
        ...
        protected T GetService<T>() where T : BaseService
        {
            ...
            service.SetupService(SpudContext, LoggingContext, UserPermissionService);
            return service;
        }
        ...
        protected T GetPostedFileAndData<T>() where T : IFileUpload
        {
            ...
            var dto = model.DeserializeJsonNet<T>();
        ...
        {
            var model = Request.Form["model"].ToString();
            var dto = model.DeserializeJsonNet<X>();
